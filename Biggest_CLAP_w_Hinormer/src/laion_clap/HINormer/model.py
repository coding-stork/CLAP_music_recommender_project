...
class REConv(nn.Module): #hetero relations encoder
    """
    Adapted from
    ...
    def __init__(self,
        ...
        self._activation = F.gelu #manually changed to avoid columns of zeros
...
class AGTLayer(nn.Module):
    ...
    def forward(self, h, rh=None):
        ...
        #print("linear final weights: ",self.linear_final.weight)
...
class HINormer(nn.Module):
    def __init__(self, g, input_dimensions, embeddings_dimension=64, num_layers=8, num_gnns=2, nheads=2, dropout=0,  temper=1.0, num_type=4, beta=1):
        ...
        #self.Prediction = nn.Linear(embeddings_dimension, 5) #removed as the task is not that of prediction/classificatio

    def forward(self, features_list, seqs, type_emb, node_type, norm=True):
        
        #print("seqs inside forward: ",seqs)

        #embeddings e altri layer custom tolti, questa è una versione di hinormer molto standard
