...
class data_loader:
    def __init__(self, path):
        ...
        self.labels_train = self.load_labels_train('label.dat')
        self.labels_test = self.load_labels_test('label.dat.test')
    ...
    def load_labels_train(self, name):
        """
        return labels dict
        ...
        """
        with open(self.path+'/labels_train.pkl', 'rb') as f:
          labels = pickle.load(f)
        labels['data'] = labels['data'][:10000]
        labels['mask'] = labels['mask'][:10000]
        return labels

    def load_labels_test(self, name):
        """
        return labels dict
            num_classes: total number of labels
            total: total number of labeled data
            ...
            mask: to indicate if that node is labeled, if False, that line of data is masked
        """
        with open(self.path+'/labels_test.pkl', 'rb') as f:
          labels = pickle.load(f)
        labels['data'] = labels['data'][:10000]
        labels['mask'] = labels['mask'][:10000]       
        return labels
    ...
    def load_links(self):
        ...
        """
        with open(self.path+'/edges_dictionary.pkl', 'rb') as f:
          links = pickle.load(f)
        new_data = {}
    ...
    def load_nodes(self):
        ...
        """
        with open(self.path+'/nodes_dictionary.pkl', 'rb') as f:
          nodes = pickle.load(f)
        return nodes
