    ...
    def forward(self, audio_features, nodes_features, logit_scale_a, logit_scale_t=None, audio_features_mlp=None, nodes_features_mlp=None):
        ...
        if self.mlp_loss:
            if self.world_size > 1:
                ...
                '''print("logits_per_audio: ",a_logits_per_audio)
                print("logits_per_nodes: ",t_logits_per_nodes)
                print("logits_per_audio.shape: ",a_logits_per_audio.shape)
                print("logits_per_nodes.shape: ",t_logits_per_nodes.shape)
                print("logits_per_audio.mean: ",torch.mean(a_logits_per_audio))
                print("logits_per_nodes.mean: ",torch.mean(t_logits_per_nodes))
                print("logits_per_audio.std: ",torch.std(a_logits_per_audio, dim=1, keepdim=True))
                print("logits_per_nodes.std: ",torch.std(t_logits_per_nodes, dim=1, keepdim=True))'''
            ...
            if self.world_size > 1:
                ...
                print("logits_per_audio.std: ",torch.std(logits_per_audio, dim=1, keepdim=True))
                print("logits_per_nodes.std: ",torch.std(logits_per_nodes, dim=1, keepdim=True))'''
            ...
            if not self.weighted_loss:
                ...
                # Check if `all_audio_features` exists, otherwise use `audio_features`
                audio_weight = (all_audio_features if 'all_audio_features' in locals() else audio_features) @ \
                              (all_audio_features.T if 'all_audio_features' in locals() else audio_features.T)
                audio_weight = (torch.exp(torch.sum(audio_weight, axis=1) / (self.weight_loss_kappa * len(audio_weight)))).detach()
                
                # Similar for text
                nodes_weight = (all_nodes_features if 'all_nodes_features' in locals() else nodes_features) @ \
                              (all_nodes_features.T if 'all_nodes_features' in locals() else nodes_features.T)
                nodes_weight = (torch.exp(torch.sum(nodes_weight, axis=1) / (self.weight_loss_kappa * len(nodes_weight)))).detach()
