...
from HINormer.utils.data import load_data
import dgl
...
def preprocess_single(
    ...
    uri_index = uri_indexes[track_uris]
    sample["index"] = uri_index
    sample["node_seq"] = all_sequences[uri_index] #get the correct node sequence
    ...
    for sample in batch:
        data_preprocessed.append(
            preprocess_single(sample, audio_ext, text_ext, max_len, audio_cfg, tmodel, class_index_dict, data_filling,
                              data_truncating, text_augment_selection, uri_indexes, all_sequences))
...
def get_wds_dataset(
    ...
    if is_train or args.parallel_eval: #ricorda che se non shuffla, è perché sta facendo validation
        pipeline.extend(
            [
                wds.detshuffle(
...
def get_data(args, device, model_cfg):
    data = {}
    ...
    print("uri_tracce: ",uri_tracce)

    #dictionary helps selecting the features by id faster
    uri_indexes = dict(zip(uri_tracce,range(len(uri_tracce))))

    #here most of the code here is taken from hinormer run.py (basically it's the dataset preprocessing)

    features_list, adjM, labels, train_val_test_idx, graph_dataloader = load_data('spotify') # load graph dataset
    ...
    #feats 0 -> feature vector, feats 3 -> one-hot encoded vector
    if feats_type == 0:
        in_dims = [features.shape[1] for features in features_list]
    ...
    node_seq = torch.zeros(features_list[0].shape[0], args.lensequence).long()
    ...
    for x in all_nodes:
        ...
        while (cnt < args.lensequence):
            sample_list = g.successors(start).cpu().numpy().tolist()
    ...
    print('node_seq_size:',node_seq.shape)
    ...
    train_seq = node_seq[:train_val_test_idx['train']]
    val_seq = node_seq[train_val_test_idx['train']:train_val_test_idx['val']]
    test_seq = node_seq[train_val_test_idx['val']:]
    ...
    if args.train_data:
        data["train"] = get_dataset_fn(args.dataset_type)(
            args, model_cfg, features_list, node_seq, node_type, type_emb, uri_indexes, is_train=True
        )

    if args.val_data:
        data["val"] = get_dataset_fn(args.dataset_type)(
            args, model_cfg, features_list, node_seq, node_type, type_emb, uri_indexes, is_train=False
        )

    print("train_seq_length: ",len(train_seq))
    print("val_seq_length: ",len(val_seq))

    data["features_list"] = features_list
    data["type_emb"] = type_emb
    data["node_type"] = node_type

    return data, g, in_dims, node_cnt #need to return more things as they are needed for model creation
