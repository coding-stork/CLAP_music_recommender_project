...
# function to track weight changes and gradients (for utility purposes)
def log_weight_gradients(model, prev_weights):
    layer_logs = []
    
    for name, param in model.named_parameters():
        if param.requires_grad:

            # cmpute gradient norm
            if param.grad is not None:
                grad_norm = param.grad.norm(2).item()
            else:
                grad_norm = 0.0
            
            # check how much the weight changed
            weight_change = torch.norm(param.data - prev_weights[name]).item()
...
# Initialize a dictionary to store previous weights
...
def train_one_epoch(
    ...
    for i, batch in enumerate(dataloader):
        ...
        model_prev_weights = get_initial_weights(model)
        ...
        #model_logs = log_weight_gradients(model, model_prev_weights)
        #for log in model_logs:
        #    print(f"[GNN] Layer: {log['layer']}, Grad Norm: {log['grad_norm']}, Weight Change: {log['weight_change']}")
        ...
        '''
        #LEFTOVER STUFF FOR DEBUG (still maybe useful)
        model_logs = log_weight_gradients(model, model_prev_weights)
        sum_audio_grads = 0
        sum_node_grads = 0
        sum_audio_wc = 0
        ...
        num_node_layers = 0
        for log in model_logs:
            if log['layer'][:5] == 'audio':
                sum_audio_grads += log['grad_norm']
                sum_audio_wc += log['weight_change']
                num_audio_layers += 1
            if log['layer'][:5] == 'graph':
                sum_node_grads += log['grad_norm']
                sum_node_wc += log['weight_change']
                num_node_layers += 1
            print(f"[GNN] Layer: {log['layer']}, Grad Norm: {log['grad_norm']}, Weight Change: {log['weight_change']}")
        print('sum_audio_grads: ',sum_audio_grads)
        print('sum_audio_wc: ',sum_audio_wc)
        print('sum_node_grads: ',sum_node_grads)
        print('sum_node_wc: ',sum_node_wc)
        print('num audio layers: ',num_audio_layers)
        print('num node layers: ',num_node_layers)
        print('mean audio layerswc: ',sum_audio_wc/num_audio_layers)
        print('mean node layerswc: ',sum_node_wc/num_node_layers)
        print('mean audio layersgrad: ',sum_audio_grads/num_audio_layers)
        print('mean node layersgrad: ',sum_node_grads/num_node_layers)
        '''
    ...
    if args.val_dataset_names == ['Clotho', 'audiocaps']:
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                #print("batch inside train: ",batch)

                '''#let's hear the audios
                print(batch)
                db_waveform = audios['waveform'][0].numpy()
                #db_waveform = db_waveform / np.max(np.abs(db_waveform))
                audio_segment = AudioSegment(
                    db_waveform.tobytes(),
                    frame_rate=48000,
                    sample_width=db_waveform.dtype.itemsize,
                    channels=1
                )

                outpath = "/content/exported_waveforms/"+batch['audio_name'][0]

                audio_segment.export(f"{outpath}.wav", format="wav")
                '''
            ...
            if is_master(args):
                ...
                _eval_chunk_size = 2048  # Adjust based on memory limits
