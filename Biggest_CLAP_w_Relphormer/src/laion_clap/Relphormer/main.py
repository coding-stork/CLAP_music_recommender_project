...
def _setup_parser():
    ...
    parser.add_argument("--accelerator", type=str, default="gpu", help="Type of accelerator (e.g., 'gpu')")
    parser.add_argument("--devices", type=int, default=1, help="Number of devices (e.g., GPUs) to use")
    parser.add_argument("--max_epochs", type=int, default=16, help="Number of epochs")
    ...
    '''
        edit by bizhen
    '''
    parser.add_argument("--max_triplet", type=int, default=8)
...
def main():
    ...
    print(f'\n \t Data_class: data.{args.data_class}')
    print(f'\n \t Model_class: models.{args.model_class}')
    print(f'\n \t Litmodel_class: lit_models.{args.litmodel_class}')
    ...
    model_checkpoint = pl.callbacks.ModelCheckpoint(monitor=metric_name, mode="max",
        ...
        dirpath="output_pretrained" if args.pretrain else "output",
    ...
    trainer = pl.Trainer(accelerator=args.accelerator, accumulate_grad_batches=args.accumulate_grad_batches, callbacks=callbacks, devices=args.devices, max_epochs=args.max_epochs, val_check_interval=args.val_check_interval, logger=logger, default_root_dir="training/logs")
    #you can also add resume_from_checkpoint=args.checkpoint
