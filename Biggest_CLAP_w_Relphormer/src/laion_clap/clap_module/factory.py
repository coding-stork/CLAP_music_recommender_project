...
def create_model(
    ...
    if pretrained == "openai":
        ...
        model_cfg["enable_fusion"] = enable_fusion
        model_cfg["fusion_type"] = fusion_type
        model_cfg["relphormer_args"] = relphormer_args
        model_cfg["relphormer_init"] = relphormer_init
        ...
        if pretrained_audio:
            if amodel_name.startswith('PANN'):
                ...
                elif os.path.basename(pretrained_audio).startswith('music'):  # custom handling for music_audioset checkpoint
                    print("loading my custom checkpoint")
                    audio_ckpt = torch.load(pretrained_audio, map_location='cpu')
                    audio_ckpt = audio_ckpt['state_dict']
                    keys = list(audio_ckpt.keys())
                    for key in keys:
                        if key.startswith('module.audio_branch.'):  # directly filter and rename keys
                            audio_ckpt[key[len("module."):]] = audio_ckpt.pop(key)
                            print(f"copying {key} to {audio_ckpt[key[len('module.'):]]}")
                else:
                    raise ValueError('Unknown audio checkpoint')
            else:
                audio_ckpt = torch.load(pretrained_audio, map_location='cpu')
