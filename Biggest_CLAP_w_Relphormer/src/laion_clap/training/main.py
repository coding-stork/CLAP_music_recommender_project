...
class DummyArgs:
  ...
  def to_dict(self):
        print("Converting DummyArgs to dictionary.")
        return self.__dict__

def dict_to_namespace(arg_dict):
    args = DummyArgs(arg_dict)
    return args
...
def main():
    ...
    relphormer_args_dict = { #too many arguments for relphormer to manually add them to the normal parser, just edit them here
        "max_epoch": 25,
        "num_workers": 8,
        "model_name_or_path": "/content/CLAP_w_Relphormer/src/laion_clap/Relphormer/Pre-trained_models/bert-base-uncased",
        "accumulate_grad_batches": 1,
        "model_class": "Relphormer",
        "batch_size": 64, #most of these are useless but let's put them anyway
        "checkpoint": "/content/hits1=0.21.ckpt", #this is important -> put your pretrained checkpoint
        "data_dir": "/content/CLAP_w_Relphormer/src/laion_clap/Relphormer/dataset/fb15k-237",
        "max_seq_length": 64,
        "warm_up_radio": 0.1,
        ...
        "lr": 1e-5,
        "label_smoothing": 0.1,
        # Add other Relphormer arguments here
    }
    relphormer_args = dict_to_namespace(relphormer_args_dict)
    ...
    #since we need some model specs to initialize the data (like embedding dim) we need to get the model_cfg before creating the mdoel
    ...
    data, tokenizer_rel, data_config = get_data(args, relphormer_args, device, model_cfg_to_load_data) #we'll need the tokenizer later as well
    print("DATA LOADED")
    ...
    print(f"Mask Token ID: {tokenizer_rel.mask_token_id}") #make sure is 103
    ...
    if args.train_data is None:
        ...
        if args.split_opt:
            ...
            #print('optimizer args 1: ',gain_or_bias_params)
            #print('optimizer args 2: ',rest_params)
